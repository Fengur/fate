...
typedef void (^FGCarouselViewTapBlock)();

@interface FGClickableImageView : UIImageView
@property (nonatomic, assign) BOOL enable;
@property (nonatomic, copy) FGCarouselViewTapBlock tapBlock;
@end

@implementation FGClickableImageView
...
@implementation FGCarouselItem

@end

@implementation FGCarouselURLItem

@end

@implementation FGCarouselViewItem
...
@property (nonatomic, copy) FGCarouselViewDidSelectBlock didSelectBlock;
@property (nonatomic, copy) FGCarouselViewDidChangeBlock didChangeBlock;
...
- (void)setImageItemsWithArray:(NSArray *)items {
    ...
    for (NSString *imageUrl in items) {
        FGCarouselURLItem *urlItem = [FGCarouselURLItem new];
    ...
    NSLog(@"%lu",(unsigned long)_items.count);
    _scrollView.scrollEnabled = _items.count > 1;

    NSInteger index = 0;
    for (FGCarouselItem *item in _items) {
        FGClickableImageView *itemView = [FGClickableImageView new];

        itemView.userInteractionEnabled = YES;
        if ([item isKindOfClass:[FGCarouselURLItem class]]) {
            NSString *imageUrl = [(FGCarouselURLItem *)item imageUrl];
//            [itemView sd_setImageWithURL:[NSURL URLWithString:imageUrl]
//                        placeholderImage:self.defaultImage];
        } else if ([item isKindOfClass:[FGCarouselViewItem class]]) {
            UIView *customView = [(FGCarouselViewItem *)item view];
...
- (void)setDidSelectBlock:(FGCarouselViewDidSelectBlock)didSelectBlock {
    _didSelectBlock = didSelectBlock;
}

- (void)setDidChangeBlock:(FGCarouselViewDidChangeBlock)didChangeBlock {
